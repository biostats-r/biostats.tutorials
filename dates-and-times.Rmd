...
R uses locales to know what language to expect the date to the written in. See `?locales`. Locales are a bit of a pain.
...
I don't think the breaks in this plot were chosen very well so I can use `scale_x_time` to set the breaks and the format for the times (this is where the codes listed in `?strptime` become useful)
