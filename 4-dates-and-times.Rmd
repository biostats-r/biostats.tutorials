...
Use the `as.Date` (for the date) and `as.POSIXct` (for the datetime) to convert the following to date or datetime format.
...
This is normally fine unless you are dealing with local times in multiple timezones, or because the you need to allow for daylight saving time.
...
R uses locales to know what language to expect the date to the written in. 
See `?locales`. 
